...
```{r}
tree.reg5=rpart(snapshot.2~.,data=data5,control=rpart.control(cp=as.numeric(attributes(which.min(CVerr5))$names)))
...
```{r}
tree.reg6=rpart(eval~.,data=data6,control=rpart.control(cp=as.numeric(attributes(which.min(CVerr6))$names)))
...
```{r}
residuals=data4$eval-predict(tree.reg4,data4)
SSE=sum(residuals^2)
SST=sum((data4$eval-mean(data4$eval))^2)
SSE/SST
```
...
Pourtant,un étudiant utilsant l'outil CYS mais étant en CMI a tendance de progresser moins bien en comparaison avec un étudiant  n'utilisant pas l'outil CYS et étant en CMI.

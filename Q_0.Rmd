...
```{r}
Q0[,'X4b']=Q0[,'X4ba']*1+Q0[,'X4bb']*2+Q0[,'X4bc']*3+Q0[,'X4bd']*4
Q0$X4b
Q0[,'X4a']=Q0[,'X4aa']*1+Q0[,'X4ab']*2+Q0[,'X4ac']*3+Q0[,'X4ad']*4
Q0$X4a
miss=which(Q0$X4a==0|Q0$X4b==0)
miss
Q0_4=Q0[-miss,c('X4a','X4b')]

Q0_4$X4a=as.factor(Q0_4$X4a,labels=c('very good','good','sufficient','bad'))
summary(Q0_4)
```
...
```
```{r}
plot(med$snapshot.1,med$snapshot.2,col=as.numeric(med$CYS.S4),xlab="snapshot.1",ylab="snapshot.2",asp=1)
abline(modbest)
```

```{r}
ggplot(med[,c(1,2)])+geom_line()
```
